...
import org.usfirst.frc.team2555.robot.*;
...
//    }
    ...
    public void update() {
        ...
        this._displacement += this._velocity * this._eTimer.get()
                + (-Robot.accelerometer.getY() * 9.80662 * Math.pow(this._eTimer.get(), 2)) / 2;
        // v = a(dt)
...
//        this._velocity += Math.round(this._accelSensor.getY() * 1000.0) / 1000.0 * this._eTimer.get();
        this._velocity += Math.round(-Robot.accelerometer.getY() * 9.80662 * 1000.0) / 1000.0 * this._eTimer.get();
